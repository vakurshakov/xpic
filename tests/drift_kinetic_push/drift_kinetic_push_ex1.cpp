...
static constexpr char help[] =
  "Test drift-kinetic pusher: larmor center (guiding center) should remain fixed\n"
  "and energy should be conserved in constant B field (E=0, gradB=0).\n";

constexpr Vector3R B0(0.0, 0.0, 2.0);

void get_magnetic_field(const Vector3R&, Vector3R& E_p, Vector3R& B_p, Vector3R& gradB_p)
{
  E_p = {0.0, 0.0, 0.0};
  B_p = B0;
  gradB_p = {0.0, 0.0, 0.0};
...
int main(int argc, char** argv)
{
  ...
  PetscCheck(equal_tol(point_n.r, start_r, 1e-10), PETSC_COMM_WORLD, PETSC_ERR_USER,
    "Larmor center must remain fixed in uniform B (result: %.10e %.10e %.10e, expected: %.10e %.10e %.10e)",
    point_n.x(), point_n.y(), point_n.z(), start_r.x(), start_r.y(), start_r.z());

  PetscReal new_E = point_n.p_parallel * point_n.p_parallel + point_n.p_perp * point_n.p_perp;
