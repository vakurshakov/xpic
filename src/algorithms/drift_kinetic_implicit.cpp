...
constexpr PetscReal alpha = 1.0 / 6.0;

PetscReal alongSBlong(PetscReal left_coeff, PetscReal left_s, //
  PetscReal right_coeff, PetscReal right_s)
{
  return alpha * (left_coeff * sfunc0(left_s) - right_coeff * sfunc0(right_s));
}

PetscReal alongSBshort(PetscReal left_coeff, PetscReal left_s, //
  PetscReal right_coeff, PetscReal right_s)
{
...
PetscReal acrossSBfunc(PetscReal left_s0, PetscReal left_sn, //
  PetscReal right_s0, PetscReal right_sn)
{
  return sfunc11(left_s0, right_sn) + sfunc11(left_sn, right_s0) +
    2 * (sfunc11(left_s0, right_s0) + sfunc11(left_sn, right_sn));
}

PetscReal acrossSEfunc(
  ...
  Vector3R& gradB_p, Vector3R& b_p, const Vector3R& Rsn, const Vector3R& Rs0)
{
  ...
  auto alongSBx = alongSBlong;
  auto alongSBy = alongSBlong;
  auto alongSBz = alongSBlong;

  if (std::abs(p_Rsn[X] - p_Rs0[X]) < PETSC_MACHINE_EPSILON) {
    alongSBx = alongSBshort;
  }
  if (std::abs(p_Rsn[Y] - p_Rs0[Y]) < PETSC_MACHINE_EPSILON) {
    alongSBy = alongSBshort;
  }
  if (std::abs(p_Rsn[Z] - p_Rs0[Z]) < PETSC_MACHINE_EPSILON) {
    alongSBz = alongSBshort;
  }
...
PetscErrorCode DriftKineticEsirkepov::decomposition(const Vector3R& Rsn,
